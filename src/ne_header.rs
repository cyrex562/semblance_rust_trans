...
//    let mut spec_name = [0; 13];
    let mut spec_name: String = String::from("");
//    sprintf(spec_name.as_mut_ptr(),
//            b"%.8s.ORD\x00" as *const u8 as *const libc::c_char, (*ne).name);
    let ne_name_cstring = CString::from_raw(ne.name);
    let ne_name_str = ne_name_cstring.to_str().unwrap_or("");
    spec_name.push_str(format!("{}.ORD", ne_name_str).as_str());
    let spec_file_path = Path::new(spec_name.as_str());
    let display = spec_file_path.display();
    //    let mut specfile =
//
//
...
//        return;
//    }
    let mut spec_file = match File::open(&spec_file_path) {
        Err(why) => panic!("failed to open specfile {}: {}", display, why.description),
        Ok(file) => file,
    };
...
//                *const libc::c_char);
    spec_file.write(b"# Generated by dump -o\n");

    let mut i = 0;
//    while (i as libc::c_uint) < (*ne).entcount {
    while i < ne.entcount {
        let entry = ne.enttab.offset(i as isize);
        let name_cstring = CString::from_raw(entry.name);
        let name_str = name_cstring.to_str().unwrap_or("");
        spec_file.write(format!("{}\t{}\n", i + 1, name_str).as_bytes());
//        if !(*(*ne).enttab.offset(i as isize)).name.is_null() {
////            fprintf(specfile,
////                    b"%d\t%s\n\x00" as *const u8 as *const libc::c_char,
////                    i + 1i32,
